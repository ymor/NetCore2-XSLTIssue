...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="recursive">
    ...
    <xsl:variable name="calculation" select="concat($result,substring($input,1,1))" />
    <xsl:message>calculation : <xsl:value-of select="$calculation"/></xsl:message>
    <xsl:message/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:call-template name="recursive">
          ...
          <xsl:with-param name="result" select="$calculation"/>
